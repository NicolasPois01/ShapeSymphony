...
export class ArenaService {
  ...
  constructor(private circleService: CircleService) {
    this.circleService.circleNewWaitingSubject.subscribe(circle => {
      this.addCircleToWaitingList(circle);
    })
    this.circleService.circleNewAliveSubject.subscribe(circle => {
      this.addCircleToAliveList(circle);
    })
    this.circleService.circleNewDeadSubject.subscribe(circle => {
      this.addCircleToDeadList(circle);
    })
  ...
  moveCircleToWaitingList(circle: Circle) {
    const arenas = this.arenaListSubject.getValue();
    arenas.forEach((arena, arenaIndex) => {
      const index = arena.circleListDead.findIndex(c => c.id === circle.id);
      if(index !== -1) {
        arena.circleListDead.splice(index, 1);
  ...
  addCircleToWaitingList(circle: Circle) {
    const activeArena = this.activeArenaSubject.getValue();
    const updatedCircleList = [...activeArena.circleListWaiting, circle];
    const updatedArena = { ...activeArena, circleListWaiting: updatedCircleList };
    if(activeArena.id === this.activeArenaSubject.getValue().id) {
      this.activeArenaSubject.next(updatedArena);
    }
  ...
  addCircleToAliveList(circle: Circle) {
    const activeArena = this.activeArenaSubject.getValue();
    const updatedCircleList = [...activeArena.circleListAlive, circle];
    const updatedArena = { ...activeArena, circleListAlive: updatedCircleList };
    if(activeArena.id === this.activeArenaSubject.getValue().id) {
      this.activeArenaSubject.next(updatedArena);
    }
  ...
  addCircleToDeadList(circle: Circle) {
    const activeArena = this.activeArenaSubject.getValue();
    const updatedCircleList = [...activeArena.circleListDead, circle];
    const updatedArena = { ...activeArena, circleListDead: updatedCircleList };
    if(activeArena.id === this.activeArenaSubject.getValue().id) {
      this.activeArenaSubject.next(updatedArena);
    }
